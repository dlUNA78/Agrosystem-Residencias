document.querySelectorAll('[data-crop-gallery]').forEach((gallery) => {
  const mainImage = gallery.querySelector('[data-crop-gallery-main]');
  const items = Array.from(
    gallery.querySelectorAll('[data-crop-gallery-item]'),
  );
  const counter = gallery.querySelector('[data-crop-gallery-counter]');
  let currentIndex = 0;
  ...
  const showImage = (index) => {
    ...
    mainImage.alt = selected.querySelector('img')?.alt || 'Imagen del cultivo';
  ...
  gallery
    .querySelector('[data-crop-gallery-prev]')
    ?.addEventListener('click', () => showImage(currentIndex - 1));
  gallery
    .querySelector('[data-crop-gallery-next]')
    ?.addEventListener('click', () => showImage(currentIndex + 1));
