...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (landsMapEl) {
    ...
    const farmCards = document.querySelectorAll("#lands-grid article[data-lat]");
    const markers = [];

    farmCards.forEach((card) => {
      const lat = parseFloat(card.dataset.lat);
      const lng = parseFloat(card.dataset.lng);
      const name = card.querySelector("h2")?.textContent?.trim() ?? "Sin nombre";
  ...
  if (btnOpenNewLandCard) btnOpenNewLandCard.addEventListener("click", openModal);
